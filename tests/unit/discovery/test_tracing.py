...
def test_map_changed_code_to_endpoints_traces_relative_imports_from_package_init(
    tmp_path: Path,
) -> None:
    service_dir = tmp_path / "service"
    service_dir.mkdir()
...
def test_map_changed_code_to_endpoints_traces_relative_module_imports(tmp_path: Path) -> None:
    service_dir = tmp_path / "service"
    service_dir.mkdir()
    (service_dir / "__init__.py").write_text("", encoding="utf-8")
    (service_dir / "payments.py").write_text(
...
@app.post("/charge")
async def charge_endpoint():
    return await payments.charge_payment()
""".strip()
        + "\n",
        encoding="utf-8",
    )
...
def test_map_changed_code_to_endpoints_traces_absolute_module_imports(tmp_path: Path) -> None:
    services_dir = tmp_path / "services"
    services_dir.mkdir()
    (services_dir / "__init__.py").write_text("", encoding="utf-8")
    (services_dir / "payments.py").write_text(
